...
5. Explicitly tells the model NOT to draw logos, brand wordmarks, business name, **tagline**, slogan, header bars, contact details, social handles, phone numbers, or footers anywhere in the poster — including as sub-headlines, decorative copy, watermarks, or anywhere else. Those strings are composited as separate overlays by a different program; the model must not duplicate them inside the image.

6. ⚠️ COMPOSITED-OVERLAY ZONES — these are critical because the branding overlay is BLENDED ONTO the image (not added as separate strips outside it). The system prompt must require that the model keep two zones of the canvas visually CLEAN (background imagery only — NO text, no headlines, no sub-headlines, no logos, no decorative copy, no people's faces, no critical subject content):
   • TOP-CENTRE STRIP — roughly the top 14% of the canvas height, centred horizontally (~50% of the width). The tenant logo overlays here.
   • BOTTOM STRIP — roughly the bottom 18% of the canvas height, full width. A translucent contact pill overlays here.
   Any text, headline, sub-headline, or hero subject placed in those zones will be COVERED by the overlay and look broken (logo on top of a tagline, contact pill on top of a slogan). The headline / hero photo / CTAs must live in the middle ~70% of the canvas.
7. Describes the output shape: ONE detailed image prompt + one short idea title (≤8 words). Make it clear the runtime model will deliver these via a tool call — you don't need to define the tool, just describe what each field is.
